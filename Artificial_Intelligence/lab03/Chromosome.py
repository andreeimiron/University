...
class Chromosome:
    ...
    def communitiesNumber(self):
        communities = []

        for _ in range(self.__problParam['network']['noNodes'] + 1):
            communities.append([])
        for i in range(self.__problParam['network']['noNodes']):
            communities[self.__repres[i]].append(i + 1)
        comm = 0
